...
- **运行时**：working=金色高速冲刺残像 → 双手压缩尾兽玉 → 九尾狐形冲击波；idle/thinking 结界=金橙 bloom 呼吸 + 衣缘火焰起伏 + 勾玉微光 + 微粒闪烁；working 另叠残像/尾兽玉脉冲/冲击环（SMIL）。

#### ⑤ `sixpaths` — 六道仙人模式（阈值 300）

- **静态**：对齐 `war/sixpaths.png` 鸣人侧——橙金外衣 / 九喇嘛披风勾玉、金瞳；**六个求道玉**环绕定位烤进图。
- **运行时**：working=金光 bloom 爆发 + 六角术式闪现 + 光刃硬切（三帧招式图更华丽：光环求道玉 → 六角封印阵 → 六道光盾/光刃）；idle/thinking 结界=求道玉环转（SMIL）+ 金色 bloom 呼吸 + 微粒闪烁。
